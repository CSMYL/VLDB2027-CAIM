    ...
    def DGL_collator(datapoints):
        ...
        for dp_id, (edge_list, node_types, edge_types, features, label) in datapoints:
            ...
            if b_features is None:
                ...
                main_node_ids.append(0)
            else:
                main_node_ids.append(len(b_features['Main_table']['INDEX_ID']))
                for node_type, g_features in features.items():
                    for feature_name, feature_values in g_features.items():
...
def get_train_val_test_datasets(dataset_name, train_test_split, encoders,
    ...
    if train_test_split == 'use_full_train':
        train_dp_ids, val_dp_ids = train_val_split(train_dp_ids, dataset_name)
        ...
        train_dp_ids, val_dp_ids = train_val_split(trainval_dp_ids, dataset_name)

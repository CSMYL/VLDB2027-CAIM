...
def get_args():
    ...
    # regularizer
    parser.add_argument('--reg_type', type=int, default=0, help='regularization type, 7: KL-Divergence, ')
    parser.add_argument('--knwl_param', default=0.1, type=float, help='the parameter to trade off knowledge, default 0.1')
    # dataset
...
def main():
    ...
    knwl_metric = None
    if args.reg_type == 7:
        knwl_metric = nn.KLDivLoss()
    elif args.reg_type == 8:
        knwl_metric = nn.MSELoss()
    else:
        print ("no knwl_metric")
    if torch.cuda.is_available(): 
        opt_metric = opt_metric.cuda()
        if knwl_metric is not None:
            knwl_metric = knwl_metric.cuda()
    ...
    for epoch in range(args.epoch):
        ...
        # train and eval
        run(epoch, model, train_loader, opt_metric, knwl_metric, plogger, optimizer=optimizer)
        auc_val = run(epoch, model, val_loader, opt_metric, knwl_metric, plogger, namespace='val')
        auc_test = run(epoch, model, test_loader, opt_metric, knwl_metric, plogger, namespace='test')
...
def run(epoch, model, data_loader, opt_metric, knwl_metric, plogger, optimizer=None, namespace='train'):
    ...
    for idx, input in enumerate(data_loader):
        ...
        if optimizer:
            y, reg_loss, knwl_loss = model(input, knwl_metric, normalization_coefficient, (epoch==6 and idx>=47))
            ...
            if idx <= 48 and idx >= 46:
                ...
                print ("knwl_loss: ", knwl_loss)
            if knwl_loss is None:
                loss = loss + reg_loss
            else:
                loss = loss + (1.0-args.knwl_param) * reg_loss + args.knwl_param * knwl_loss
            ...
            with torch.no_grad():
                ...
                if idx <= 48 and idx >= 46:
                    ...
                    print ("no grad reg loss: ", reg_loss)
                    print ("no grad knwl loss: ", knwl_loss)
                if knwl_loss is None:
                    loss = loss + reg_loss
                else:
                    loss = loss + (1.0-args.knwl_param) * reg_loss + args.knwl_param * knwl_loss

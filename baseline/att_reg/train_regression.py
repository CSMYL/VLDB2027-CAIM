...
def main():
    ...
    # optimizer
    opt_metric = nn.MSELoss(reduction='mean')
    ...
    for epoch in range(args.epoch):
        ...
        mse_val = run(epoch, model, val_loader, opt_metric, plogger, namespace='val')
        mse_test = run(epoch, model, test_loader, opt_metric, plogger, namespace='test')

        # record best aue and save checkpoint
        if mse_val <= best_mse:
            best_mse = mse_val
            plogger.info(f'best mse: valid {mse_val:.4f}, test {mse_test:.4f}')
        else:
            patience_cnt += 1
            plogger.info(f'valid {mse_val:.4f}, test {mse_test:.4f}')
            plogger.info(f'Early stopped, {patience_cnt}-th best mse at epoch {epoch-1}')
...
def run(epoch, model, data_loader, opt_metric, plogger, optimizer=None, namespace='train'):
    ...
    loss_avg, mse_avg = AverageMeter(), AverageMeter()
    ...
    for idx, input in enumerate(data_loader):
        ...
        mse_avg.update(np.sqrt(loss.item()), target.size(0))

        # tensorboardX
        writer.add_scalar(f'{namespace}/mse-step', mse_avg.val, idx+len(data_loader)*epoch)
        ...
        if idx % args.report_freq == 0:
            plogger.info(f'Epoch [{epoch:3d}/{args.epoch:3d}][{idx:3d}/{len(data_loader):3d}]\t'
                         ...
                         f'MSE {mse_avg.val:4f} ({mse_avg.avg:4f})\t'
    ...
    plogger.info(f'{namespace}\tTime {timeSince(s=time_avg.sum):>12s}\t'
                 f'MSE {mse_avg.avg:8.4f}\tLoss {loss_avg.avg:8.4f}')
    writer.add_scalar(f'{namespace}/mse-epoch', mse_avg.avg, epoch)
...
start_time, best_mse, base_exp_name = time.time(), float('inf'), args.exp_name
for args.seed in range(args.seed, args.seed+args.repeat):
    ...
    main()
    start_time, best_mse = time.time(), float('inf')
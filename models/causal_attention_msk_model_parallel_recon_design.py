...
class MaskedTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        diag_mask = torch.eye(num_features, device=scores.device).unsqueeze(0).unsqueeze(0)
        scores = scores.masked_fill(diag_mask.bool(), float('-inf'))
        
        attn_weights = F.softmax(scores, dim=-1)
        
        attn_weights = attn_weights * (1 - diag_mask)
...
class CausalAttentionMskModel(nn.Module):
    ...
    def get_embeddings(self, x, mask_positions=None):
        ...
        if mask_positions is not None:
            batch_size, num_masked = mask_positions.shape
            
            batch_indices = torch.arange(batch_size, device=x.device).unsqueeze(1).expand(-1, num_masked)
            valid_positions = mask_positions < F
            
            if valid_positions.any():
                valid_batch = batch_indices[valid_positions]
                valid_pos = mask_positions[valid_positions]
                
                feat_embed[valid_batch, valid_pos] = self.mask_embedding.squeeze(0).squeeze(0)
    ...
    def forward(self, x, mask_positions=None, target_idx=None, task_type='reconstruction'):
        ...
        if self.num_cont > 0:
            cont_encoded = encoded[:, self.cont_idx, :]
            if self.share_embedding:
                cont_outputs = self.cont_decoder(cont_encoded)
                for i, idx in enumerate(self.cont_idx):
                    outputs[idx] = cont_outputs[:, i]
            else:
                cont_outputs = torch.stack([self.cont_decoders[i](cont_encoded[:, i, :]).squeeze(-1) 
                                          for i in range(self.num_cont)], dim=1)
                for i, idx in enumerate(self.cont_idx):
                    outputs[idx] = cont_outputs[:, i]
        
        if self.num_cat > 0:
            cat_encoded = encoded[:, self.cat_idx, :]
            if self.share_embedding:
                cat_outputs = self.cat_decoder(cat_encoded)
                for i, idx in enumerate(self.cat_idx):
                    outputs[idx] = cat_outputs[i]
            else:
                cat_outputs = [self.cat_decoders[i](cat_encoded[:, i, :]) for i in range(self.num_cat)]
                for i, idx in enumerate(self.cat_idx):
                    outputs[idx] = cat_outputs[i]

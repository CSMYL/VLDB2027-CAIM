...
class CausalPredictor(nn.Module):
    ...
    def forward(self, encoded_features, mask, target_idx):
        ...
        if self.target_idx is not None and self.target_idx < self.num_features:
            ...
            non_label_mask = torch.ones(self.num_features, dtype=torch.bool, device=feature_weights.device)
            non_label_mask[self.target_idx] = False
            feature_weights[non_label_mask] = torch.relu(feature_weights[non_label_mask])
